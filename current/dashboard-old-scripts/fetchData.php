...
try {
    ...
    // TODO: Break up larger data queries into small chunks, then merge them together.
    // Whether this needs to be done via php or js is still in development
    ...
    // Testing this commented code
    // This little snippet displays the chart minutely
    $groupInterval = 60;  //Data shown for all 60 sec of a minute
    // Check if the interval is exactly one day
    // Minutely Dashboard Intervals
    ...
    // TODO: Look into if we're vulnerable to SQL injection attacks
    $stmt = $pdo->prepare("
        SELECT
            DATE_FORMAT(DATE_SUB(date_time, INTERVAL FLOOR(SECOND(date_time) / :interval) * :interval SECOND), '%Y-%m-%d %H:%i:%s') AS interval_time,
            solar_percentage AS solar_percentage,
            wind_percentage AS wind_percentage,
            hydro_percentage AS hydro_percentage,
            battery_percentage AS battery_percentage,
            solar_fixed_percentage AS solar_fixed_percentage,
            solar_360_percentage AS solar_360_percentage
