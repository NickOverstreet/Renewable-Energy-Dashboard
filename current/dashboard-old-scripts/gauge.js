...
function createGaugeChart(value, svgId) {
  ...
  function redraw() {
    svg.innerHTML = ""; // Clear the SVG
    const width = svg.clientWidth || 250;
    const height = width; // Make the SVG square

    font_scale = 0;
    // Add more precise scaling logic
    switch (true) {
      // 4K display, small
      case window.innerWidth >= 3840 && window.innerWidth <= 600:
        font_scale = 1.75 * 5;
        break;
      ...
      // Normal scaling, very narrow
      case window.innerWidth <= 300:
        font_scale = 1.55;
        break;

      // Normal scaling, small
    ...
    console.log("Font scale: ", font_scale);
    ...
    const arcPath = describeArc(
      ...
      centerY,
      radius + 10,
      (startAngle * 180) / Math.PI + 89.5,
      (endAngle * 180) / Math.PI + 90.5,
    );
    ...
    for (let i = 0; i <= 100; i++) {
      ...
      if (i % 10 === 0) {
        ...
        // Update font based on screen size
        label.setAttribute("font-size", Math.ceil(14 * font_scale));
        ...
        ); // Set text shadow using style attribute
    ...
    valueText.textContent = Math.ceil(value) + "%";
    // Add Gauge text under here so it's centered with the dial each time
...
function setGaugeLabel(id, text) {
  // console.log("setGaugeLabel called with: ", id, text);
  ...
  if (next && next.classList.contains("gauge-label")) {
    next.remove();
  }
