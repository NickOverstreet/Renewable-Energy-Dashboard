...
async function fetchData(startDate, endDate, startTime) {
    try {
        const response = await fetch(`fetchData.php?startDate=${startDate}&endDate=${endDate}&startTime=${startTime}`);
 ...
 // Convert times to milliseconds if needed
        fetchedData.interval_times = fetchedData.interval_times.map(time => new Date(time).getTime());
